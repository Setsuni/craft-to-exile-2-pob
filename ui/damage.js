...
function rateOf(spellId, sheet) {
  ...
  /* Every projectile is its own hit, and the game counts them that way: a
     ...
     But that only holds when the projectiles all go to the SAME place. Two
     tags say they do not:

       `chaining`       - the projectile ricochets on to other targets
       `random_spread`  - the projectiles scatter

     Against one target - which is what a dummy parse measures, and what a DPS
     figure is quoted against - such a skill lands one projectile and the rest
     go elsewhere. Ricochet Shot is the case that exposed this: a 53.2s parse
     logged 149 hits, 2.80/s, against our 3.089 CASTS per second. Counting all
     5.44 projectiles gave 16.82 hits/s and a DPS six times the real one.

     Magic Missile carries neither tag, which is why its parse showed every
     projectile connecting, so the two measurements agree with the rule rather
