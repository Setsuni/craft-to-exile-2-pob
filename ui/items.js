...
/* A slot the catalogue does not list still has bases.

   `EXTRA_SLOTS` exists for equipped gear whose type no catalogue slot claims -
   the elytra is the only one on this pack - and slotDef() returned a bare
   `{bases: []}` for it. That is not merely cosmetic: paintSelectors() checks
   whether the current unique is valid for the slot, finds an empty list, and
   CLEARS it. So every repaint wiped the elytra's unique, its stats went to
   zero, and changing its socketed gem could not move the sheet because the
   item had already stopped contributing anything.

   Fall back to what the base catalogue itself says: every base whose `slot` is
   this id. `ring1`/`ring2` style suffixes are stripped so a numbered slot
   finds its own type. */
const SLOT_FALLBACK = {};
function fallbackSlot(id) {
  if (SLOT_FALLBACK[id]) return SLOT_FALLBACK[id];
  const key = String(id).replace(/\d+$/, '');
  const bases = Object.keys(CAT.bases || {})
    .filter(b => (CAT.bases[b] || {}).slot === key || b === key);
  return (SLOT_FALLBACK[id] = { id: id, bases: bases });
}
const slotDef = id => EQUIP.filter(e => e.id === id)[0] || fallbackSlot(id);
...
function paintImplicit() {
  ...
  const opts = ['<option value="">— none —</option>'].concat(
    pool.map(id => '<option value="' + id + '"' + (id === e.id ? ' selected' : '') + '>' +
      affixName(id) + oddsText(id, pool) +
      '  (' + fullRange(CAT.affixes[id], cur.ilvl) + ')</option>')).join('');
  let body = '';
  if (e.id) {
...
/* How likely an affix is to land, given the pool it rolls from.

   Every affix carries a `weight` and the game picks from the pool in
   proportion to it, so an affix's odds are its weight over the pool's total.
   The weights in this pack are 50, 250, 500, 1000, 2000 and 5000 - a 5000 is
   a hundred times likelier than a 50, which is not something you can guess
   from the name.

   Weight 0 means it CANNOT DROP. Seventeen affixes sit in that state, left in
   the registry after being retired - `enchantment_gear_def`, the old 8-24%
   gear defense, among them. They are still listed in the pools, so without
   this you can build an item around a mod that no longer exists. */
function affixOdds(id, pool) {
  const w = (CAT.affixes[id] || {}).weight;
  if (!w) return { dead: true };
  let total = 0;
  (pool || []).forEach(x => { total += (CAT.affixes[x] || {}).weight || 0; });
  return total ? { pct: (w / total) * 100, weight: w } : { weight: w };
}
function oddsText(id, pool) {
  const o = affixOdds(id, pool);
  if (o.dead) return ' — cannot drop';
  if (o.pct === undefined) return '';
  return '  ' + (o.pct >= 1 ? o.pct.toFixed(1) : o.pct.toFixed(2)) + '%';
}
...
function affixRow(kind, i, entry, pool, disabled) {
  ...
  if (entry.id) {
    ...
    body = '<div class="afctl">' +
      ...
      (function () {
        const o = affixOdds(entry.id, pool);
        return o.dead
          ? '<div class="afodds dead">this mod can no longer drop</div>'
          : o.pct === undefined ? ''
          : '<div class="afodds">' + (o.pct >= 1 ? o.pct.toFixed(1) : o.pct.toFixed(2)) +
            '% of this pool · weight ' + o.weight + '</div>';
      })() +
      '<div class="afval">' +
  ...
  if (ipool.length) {
    ...
    document.getElementById('f-inf').innerHTML =
      '<option value="">\u2014 none \u2014</option>' + ipool.map(id =>
        ...
        affixName(id) + oddsText(id, ipool) + '  (' + fullRange(CAT.affixes[id], cur.ilvl) + ')</option>').join('');
    const e = cur.inf;
    document.getElementById('infmods').innerHTML = e.id
      ? '<div class="afctl" style="grid-template-columns:minmax(0,1fr) 38px">' +
        ...
        ' \u00b7 imported rolls use the infusion rarity; adjust to plan upgrades</span>' +
        (affixOdds(e.id, ipool).dead
          ? '<div class="afodds dead">this enchantment can no longer drop '
            + '— it is retired, and kept only so old items still read '
            + 'correctly</div>'
          : '') + '</div>'
      : '';
...
function paintCodex() {
  ...
  box.innerHTML = '<div class="cdx">' +
    ...
    codexAffixEditor() + wornCodex() + '</div>';
...
/* The codex's ROLLED affixes - the part the editor never offered.

   A codex carries its registry mods plus up to `affixes.max` rolled ones, and
   each sits at its own piece count. Shawn's Codex of Spite has magic find on
   it that the planner could not express at all: you could pick the codex and
   its diversity, and nothing else.

   The piece count is shown, not chosen. It falls out of the order, because
   that is how the game assigns it - see codexTiers(). */
function codexAffixEditor() {
  const pool = codexPool(cur);
  if (!pool.length) return '';
  const d = CAT.codexDifficulty[cur.codexRarity] || {};
  const maxAff = (d.affixes || {}).max || 0;
  const rows = (cur.codexAff || []).slice(0, Math.max(maxAff, (cur.codexAff || []).length));
  const total = codexTotal(cur);
  const tierOf = i => Math.max(1, total - rows.filter(a => a && a.id).length + i);
  const opts = (sel) => '<option value="">— empty —</option>' +
    pool.map(id => '<option value="' + id + '"' + (id === sel ? ' selected' : '') +
      '>' + affixName(id) + '  (' + fullRange(CAT.affixes[id], cur.ilvl) + ')</option>').join('');
  return '<div class="pool"><h4>Codex affixes ' +
    '<em>' + rows.filter(a => a && a.id).length + ' of ' + maxAff +
    ' at ' + label(cur.codexRarity) + '</em></h4>' +
    '<p class="note" style="margin:0 0 6px">Each one sits at its own piece ' +
    'count, lowest first. The registry bonus always sits at the top.</p>' +
    rows.map((a, i) =>
      '<div class="afrow"><select data-cdxa="' + i + '">' + opts(a && a.id) + '</select>' +
      ((a && a.id) ? '<div class="afctl">' +
        '<b class="pts">' + tierOf(i) + ' Piece</b>' +
        '<input type="range" min="0" max="100" value="' + (a.pct === undefined ? 100 : a.pct) +
        '" data-cdxr="' + i + '"><b>' + (a.pct === undefined ? 100 : a.pct) + '%</b></div>' +
        '<div class="afval">' + (CAT.affixes[a.id].stats || [])
          .map(m => modText(exactOf(m, a.pct === undefined ? 100 : a.pct, cur.ilvl)))
          .join(', ') + '</div>' : '') + '</div>').join('') +
    (rows.filter(a => a && a.id).length < maxAff
      ? '<div class="afrow"><select data-cdxa="' + rows.length + '">' + opts('') +
        '</select></div>' : '') + '</div>';
}
function wireCodexAffixes() {
  document.querySelectorAll('[data-cdxa]').forEach(sel => sel.onchange = () => {
    delete cur.codexOmen;
    cur.blank = false;
    cur.codexAff = cur.codexAff || [];
    const i = +sel.dataset.cdxa;
    if (!sel.value) cur.codexAff.splice(i, 1);
    else if (i >= cur.codexAff.length) cur.codexAff.push({ id: sel.value, pct: 100 });
    else cur.codexAff[i] = { id: sel.value, pct: cur.codexAff[i].pct === undefined
      ? 100 : cur.codexAff[i].pct };
    applyNow();
  });
  document.querySelectorAll('[data-cdxr]').forEach(r => rollSlider(r, v => {
    delete cur.codexOmen;
    cur.codexAff[+r.dataset.cdxr].pct = v;
  }, paintCodex));
}
...
/* A codex is a LADDER, not one bonus. The registry's own `mods` sit at the top
   tier - the full requirement total - and each rolled affix sits one tier
   below it, in order. A codex needing {NORMAL 2, UNIQUE 2, RUNED 3} tops out
   at 7 and reads 5 / 6 / 7 Piece.

   The piece count is DERIVED from that order rather than chosen: the game has
...
function codexTiers(it) {
  ...
  const out = affs.map((a, i) => ({
    ...
    mods: ((CAT.affixes[a.id] || {}).stats || [])
      .map(m => exactOf(m, a.pct === undefined ? 100 : a.pct, it.ilvl)),
  }));
  const pct = Math.min(it.codexPct, 100);
  out.push({ pieces: total, mods: c.mods.map(m => ({ stat: m.stat, type: m.type,
...
function codexPool(it) {
  const c = CAT.codex[it.codex];
  if (!c) return [];
  const want = new Set(c.affix_types || []);
  return Object.keys(CAT.affixes)
    .filter(id => want.has(CAT.affixes[id].type) && CAT.affixes[id].weight)
    .sort((a, b) => affixName(a).localeCompare(affixName(b)));
}
...
function groupsFor(it) {
  ...
  if (it.kind === 'unique') {
    /* Same shape as the normal path's `block`, but the unique branch returns
       before it is defined. */
    const uniBlock = (entries, label_, kind) => {
      const live_ = (entries || []).filter(e => e && e.id);
      if (!live_.length) return;
      groups.push({ label: label_, kind: kind, note: live_.length + ' rolled',
        lines: live_.map(e => {
          const def = CAT.affixes[e.id];
          return { name: affixName(e.id),
                   meta: (e.tier ? titleCase(e.tier) + ' · ' : '') + (e.pct || 0) + '%',
                   mods: (def ? def.stats : []).map(m => exactOf(m, e.pct, it.ilvl)) };
        }) });
    };
    ...
    /* A unique carries corruptions and an infusion like anything else, and
       they were reaching the SHEET but never the card - so Mara's Kaleidoscope
       showed its unique lines and silently dropped "+15% Skill Speed" and
       "+14% Magic Find", which is exactly what the in-game tooltip prints
       under Corruption Stats. */
    uniBlock(it.cor, 'Corruption Stats', 'corrupt');
    uniBlock([it.inf], 'Infused', 'infusion');
  ...
  block(it.cor, 'Corruption Stats', 'corrupt');
  block([it.inf], 'Infused', 'infusion');
  return groups;
}

/* What the tooltip leads with: the level and attributes this item demands,
   ticked when this character meets them.

   A base stores its requirement as a MULTIPLIER - `{strength: 0.4}` - which
   the game scales by STAT_REQ at the item's level. At level 100 that curve is
   2 + 2*(100-1) = 200, so 0.4 becomes "Strength Min: 80", which is exactly
   what the in-game tooltip prints. */
function reqBlock(it) {
  if (isJewel(it.slot) || isCodex(it.slot)) return '';
  const b = CAT.bases[it.base];
  if (!b) return '';
  const rows = [];
  const lvlOk = charLevel >= it.ilvl;
  rows.push([lvlOk, 'Player Level Min: ' + it.ilvl]);
  const scale = IE.multi('STAT_REQ', it.ilvl);
  Object.entries(b.req || {}).forEach(([attr, mult]) => {
    const need = Math.round(mult * scale);
    if (!need) return;
    rows.push([live.total(attr) >= need, titleCase(attr) + ' Min: ' + need]);
  });
  return '<div class="itemreq">' + rows.map(([ok, txt]) =>
    '<div class="' + (ok ? 'met' : 'unmet') + '">' +
    (ok ? '✓' : '✗') + ' ' + txt + '</div>').join('') + '</div>';
}
